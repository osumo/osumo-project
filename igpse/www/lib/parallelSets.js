var parallelSets = (function () {
    ...
    var return_sourceKey = function(source){
      for (var m=0; m<_data[0].length;m++ ){
        if(_data[0][m].key == source)
        {return _data[0][m].key;}
      }
    }
    ...
    var linkLine = function(start) {
        return function(l) {
          ...
          var source = returnObject(l.source, 0),
              target = returnObject(l.target, 1),
              gapWidth = x(0)-3,
              bandWidth = x.rangeBand() + gapWidth,
              startx = x.rangeBand() - bandWidth,
              sourcey = y(source.offsetValue) + source.order * settings.padding + y(l.outOffset) + y(l.count)/2,
      ...
      nodes.append('rect').attr('fill', function( d ){
           ...
           .on('click', function(d){
                    ...
                    GROUP1.node.push(d.key);
                  }
        				  if(group_f == 2){
                    d3.select(this).attr('class', 'box red');
                    GROUP2.node.push(d.key);
                  }
            });
                 ...
                 .on("click", function(l){
                        if(group_f == 1){
                          d3.select(this).attr('class', 'link two');
                            GROUP1.link.push({source: l.source, target: l.target});
                        }
                        if(group_f == 2){
                          d3.select(this).attr('class', 'link one');
                            GROUP2.link.push({source: l.source, target: l.target});
                        }
                 	});
		};
